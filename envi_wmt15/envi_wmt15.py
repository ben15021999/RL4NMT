...
# English to Vietnamese translation datasets.
_ENVI_TRAIN_DATASETS = [
    'train.en',
    'train.vi'
]

_ENVI_STRAIN_DATASETS = [
]
...
class TranslateEnviWmt15(TranslateProblem):
    """Problem spec for WMT15 En-Vi translation."""
    ...
    def generator(self, data_dir, tmp_dir, train):
        ...
        print('    ' + _ENVI_VOCAB_FILES[0] + ' ... ', end='')
        sys.stdout.flush()
        with open(os.path.join(data_dir, _ENVI_VOCAB_FILES[0]), 'rb') as f:
        ...
        print('    ' + _ENVI_VOCAB_FILES[1] + ' ... ', end='')
        sys.stdout.flush()
        with open(os.path.join(data_dir,_ENVI_VOCAB_FILES[1]), 'rb') as f:
        ...
        datapath = _ENVI_TRAIN_DATASETS if train else _ENVI_DEV_DATASETS
    ...
    def feature_encoders(self, data_dir):
        # Load source vocabulary.
        tf.logging.info("Loading and processing source vocabulary from: %s" % _ENVI_VOCAB_FILES[0])
        with open(os.path.join(data_dir,_ENVI_VOCAB_FILES[0]), 'rb') as f:
            vocab_src_list = f.read().decode('utf8', 'ignore').splitlines()
        tf.logging.info("Done")
        
        # Load target vocabulary.
        tf.logging.info("Loading and processing target vocabulary from: %s" % _ENVI_VOCAB_FILES[1])
        with open(os.path.join(data_dir,_ENVI_VOCAB_FILES[1]), 'rb') as f:
            vocab_trg_list = f.read().decode('utf8', 'ignore').splitlines()
        tf.logging.info("Done")
...
def envi_wmt15_transformer_rl_delta_setting():
...
@registry.register_hparams
def envi_wmt15_transformer_rl_delta_setting_random():
...
@registry.register_hparams
def envi_wmt15_transformer_rl_delta_setting_random_mrt():
...
@registry.register_hparams
def envi_wmt15_transformer_rl_total_setting():
...
@registry.register_hparams
def envi_wmt15_transformer_rl_total_setting_random():
...
@registry.register_hparams
def envi_wmt15_transformer_rl_delta_setting_random_baseline():
    hparams = transformer.transformer_big()
    hparams.shared_embedding_and_softmax_weights = 0
    hparams.layer_prepostprocess_dropout = 0.05
    hparams.learning_rate = 0.1
...
@registry.register_hparams
def envi_wmt15_transformer_rl_delta_setting_random_mle():
    hparams = transformer.transformer_big()
    hparams.shared_embedding_and_softmax_weights = 0
    hparams.layer_prepostprocess_dropout = 0.05
    hparams.learning_rate = 0.1
